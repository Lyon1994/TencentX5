...
import com.tencent.x5.R;
import com.tencent.x5.utils.X5WebView;
...
public class X5FirstTimeActivity extends Activity{
	
	
	/**
	 * 本例展示如何第一次打开X5webview 就能加载X5内核的方法
	 * 将适用于特殊的场景
	 * 但是注意 额外的冷启动时间
	 */
		...
		this.handler = new Handler(Looper.getMainLooper()){
			...
			// TODO Auto-generated method stub
				...
				case MSG_WEBVIEW_CONSTRUCTOR:
					X5FirstTimeActivity.this.doX5WebViewConstruction();
					break;
				case MSG_WEBVIEW_POLLING:
					X5FirstTimeActivity.this.polling();//循环查询X5内核是否加载
					break;
				}
	...
	private void preinitX5WebCore(){
		...
		if(!QbSdk.isTbsCoreInited()){//preinit只需要调用一次，如果已经完成了初始化，那么就直接构造view
			QbSdk.preInit(X5FirstTimeActivity.this, myCallback);//设置X5初始化完成的回调接口  第三个参数为true：如果首次加载失败则继续尝试加载；
		}else{
	...
	private PreInitCallback myCallback=new PreInitCallback() {
		...
		public void onViewInitFinished() {//当X5webview 初始化结束后的回调
			// TODO Auto-generated method stub
			float deltaTime=(System.currentTimeMillis()-X5FirstTimeActivity.this.timerCounter)/1000;
			Toast.makeText(X5FirstTimeActivity.this, "x5初始化使用了"+deltaTime+"秒", Toast.LENGTH_LONG).show();
			Log.i("yuanhaizhou", "x5初始化使用了"+deltaTime+"秒");
			X5FirstTimeActivity.this.polling();
		}
